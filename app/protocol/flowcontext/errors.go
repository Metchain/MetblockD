...
func (*FlowContext) HandleError(err error, flowName string, isStopping *uint32, errChan chan<- error) {
	isErrRouteClosed := errors.Is(err, routerpkg.ErrRouteClosed)
	if !isErrRouteClosed {
...
func (*FlowContext) IsRecoverableError(err error) bool {
	return err == nil || errors.Is(err, routerpkg.ErrRouteClosed) || errors.As(err, &protocolerrors.ProtocolError{})
}
