...
type Peer struct {
	...
	ibdRequestChannel chan *external.DomainBlock // A channel used to communicate IBD requests between flows
}
...
func New(connection *netadapter.NetConnection) *Peer {
	return &Peer{
		...
		ibdRequestChannel: make(chan *external.DomainBlock),
	}
}
...
func (p *Peer) SubnetworkID() *external.DomainSubnetworkID {
...
func (p *Peer) IBDRequestChannel() chan *external.DomainBlock {
