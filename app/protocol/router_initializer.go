...
import (
	"github.com/Metchain/MetblockD/app/metflow"
	"github.com/Metchain/MetblockD/app/metflow/ready"
	"github.com/Metchain/MetblockD/app/protocol/common"
	"github.com/Metchain/MetblockD/app/protocol/flowcontext/handshake"
...
func (m *Manager) routerInitializer(router *routerpkg.Router, netConnection *netadapter.NetConnection) {
	...
	spawn("routerInitializer-runFlows", func() {
		...
		case 5:
			flows = metflow.Register(m, router, errChan, &isStopping)
		default:
...
func (m *Manager) handleError(err error, netConnection *netadapter.NetConnection, outgoingRoute *routerpkg.Route) {
	if protocolErr := (protocolerrors.ProtocolError{}); errors.As(err, &protocolErr) {
		if m.context.Config().EnableBanning && protocolErr.ShouldBan {
			...
			if err != nil && !errors.Is(err, commanager.ErrCannotBanPermanent) {
				panic(err)
			}
