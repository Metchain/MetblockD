...
const (
	// PubKey addresses always have the version byte set to 2531809093.
	pubKeyAddrKey = 0x96e85745 //2531809093

	// PubKey addresses always have the version byte set to 1.
	pubKeyECDSAAddrKey = 0x01

	// ScriptHash addresses always have the version byte set to 8.
	scriptHashAddrKey = 0x08
)
const (
	PubkeyAddrId = 0x00
...
// and a payload which encodes the Metchain network and address type. It is used
...
func DecodeAddress(addr string, expectedPrefix Bech32Prefix) (Address, error) {
	...
	case pubKeyECDSAAddrKey:
		return newAddressPubKeyECDSA(prefix, decoded)
	case scriptHashAddrKey:
...
func (a *AddressPublicKey) EncodeAddress() string {
	return encodeAddress(a.prefix, a.publicKey[:], a.pubVersion.encodePubkeyVersion())
}

func (a *PubVersionkey) encodePubkeyVersion() []byte {

	return a.econdePubkeytoByte()
}

func (a *PubVersionkey) econdePubkeytoByte() []byte {
	return []byte(a.convertIntTostring())
}

func (a *PubVersionkey) convertIntTostring() string {
...
// with the passed Metchain network.
...
func (a *AddressPublicKey) Version() []byte {
	return a.pubVersion.encodePubkeyVersion()
}
...
func (a *AddressScriptHash) EncodeAddress() string {
	return encodeAddress(a.prefix, a.hash[:], a.pubVersion.encodePubkeyVersion())
}
