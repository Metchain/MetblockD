...
func (c *gRPCConnection) receiveLoop() error {
	...
	for c.IsConnected() {
		...
		log.Infof("MSG:", protoMessage)
		log.Infof("MSG Number:", messageNumber)
		//Update for message handleing
...
func (c *gRPCConnection) sendLoop() error {
	...
	for c.IsConnected() {
		...
		blockDelayOnce.Do(func() {
			experimentalDelayEnv := os.Getenv("METCHAIN_EXPERIMENTAL_DELAY")
