...
func (h HashWriter) Finalize() *external.DomainHash {
	var sum [external.DomainHashSize]byte
	...
	return external.NewDomainHashFromByteArray(&sum)
}
...
// Finalize returns the resulting hash
func (h *ShakeHashWriter) Finalize() *external.DomainHash {
	var sum [external.DomainHashSize]byte
	// This should prevent `Sum` for allocating an output buffer, by using the DomainHash buffer. we still copy because we don't want to rely on that.
	_, err := h.Read(sum[:])
	if err != nil {
